...
os.chdir(r"C:\Users\AVShestakov\HLD_sectorKey\Волна 2.3\Центр")
...
# BSSI выделяем 4G
df_BSSI_4G = df_BSSI[df_BSSI['Стандарт'].str.contains('LTE')]
...
df_BSSI_check = df_BSSI_4G[
    ['Стандарт', 'Филиал', 'Номер позиции', 'Band', 'LTE BW, MHz', 'Σ Network Elements cummulative',
     'Строительство/Модернизация/Демонтаж', 'Worktype', 'Sector_key']].copy()


def newsite1800(row):
    if ((row['Band'] == 1800) & (row['LTE BW, MHz'] != '4T4R') & (row['Σ Network Elements cummulative'] > 0)
            & (row['Строительство/Модернизация/Демонтаж'].lower() == 'строительство') & (row['Worktype'] == 1)):
        val = 1
    else:
        val = 0
    return val
...
def entrance1800(row):
    if ((row['Band'] == 1800) & (row['LTE BW, MHz'] != '4T4R') & (row['Σ Network Elements cummulative'] > 0) &
            (row['Строительство/Модернизация/Демонтаж'].lower() == 'модернизация')
            & (row['Worktype'].lower() == 'въезд')):
        val = 1
    else:
        val = 0
    return val
...
def entrance2100(row):
    if ((row['Band'] == 2100) & (row['LTE BW, MHz'] != '4T4R') & (row['Σ Network Elements cummulative'] > 0) &
            (row['Строительство/Модернизация/Демонтаж'].lower() == 'модернизация')
            & (row['Worktype'].lower() == 'въезд')):
        val = 1
    else:
...
def entrance2600(row):
    if ((row['Band'] == 2600) & (row['LTE BW, MHz'] != '4T4R') & (row['Σ Network Elements cummulative'] > 0) &
            (row['Строительство/Модернизация/Демонтаж'].lower() == 'модернизация')
            & (row['Worktype'].lower() == 'въезд')):
        val = 1
    else:
...
def entrance2600TDD(row):
    if ((row['Band'] == '2600 TDD') & (row['LTE BW, MHz'] != '4T4R') & (row['Σ Network Elements cummulative'] > 0) &
            (row['Строительство/Модернизация/Демонтаж'].lower() == 'модернизация')
            & (row['Worktype'].lower() == 'въезд')):
        val = 1
    else:
...
def MIMO(row):
    if ((row['LTE BW, MHz'] == '4T4R') & (
            (row['Σ Network Elements cummulative'] == 0) | pd.isnull(row['Σ Network Elements cummulative'])) &
            (row['Строительство/Модернизация/Демонтаж'].lower() == 'модернизация')
            & (row['Worktype'].lower() == 'MIMO 4T4R')):
        val = 1
    else:
        val = 0
    return val
...
def bisector(row):
    if ((row['LTE BW, MHz'] != '4T4R') & (row['Σ Network Elements cummulative'] == 0) & (
    pd.isnull(row['Σ Network Elements cummulative']))
            & (row['Строительство/Модернизация/Демонтаж'].lower() == 'модернизация')
            & (row['Worktype'].lower() == 'Bi-Sector')):
        val = 1
    else:
        val = 0
    return val
...
df_BSSI_check3.loc[df_BSSI_check3['2600 Entrance'] > 1, '2600 Entrance'] = 1
df_BSSI_check3.loc[df_BSSI_check3['2100 Entrance'] > 1, '2100 Entrance'] = 1
...
result = result.replace(['left_only', 'right_only'],
...
#     & (df_BSSI_check['Σ Network Elements cummulative'] > 0) & (df_BSSI_check['Worktype'].str.contains('оител'))]
#
...
#     (df_BSSI_check['Band'] == 1800) & (df_BSSI_check['LTE BW, MHz'] != '4T4R')
#     & (df_BSSI_check['Σ Network Elements cummulative'] > 0) & (df_BSSI_check['Worktype'].str.contains('дернизац'))]
#
...
#                              & ((df_BSSI_check['Σ Network Elements cummulative'] == 0) |
#                                 df_BSSI_check['Σ Network Elements cummulative'].isnull()) & (
